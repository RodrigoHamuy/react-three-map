...
/** init `useR3M` hook */
export function useInitR3M<T extends MapInstance>(map: T, fromLngLat: FromLngLat) {
  ...
  if(prevMap !== map) initR3M(map, fromLngLat, store);
}

export function initR3M<T extends MapInstance>(map: T, fromLngLat: FromLngLat, store: UseBoundStore<RootState>) {
  const viewProjMx = new Matrix4().identity().toArray();
  const r3m : R3M<T> = {map, fromLngLat, viewProjMx };
